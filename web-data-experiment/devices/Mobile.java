...
public class Mobile{
    ...
    //overloading
    public void outputData(byte[] webcontent, String path, String filename){
    	PrintWriter out;
    	//Create directory and file
		try {
			File dir = new File(path + "/ReconstructedBytes/");
			dir.mkdir();
			out = new PrintWriter(path + "/ReconstructedBytes/" + filename + "-recon");
    	int len = webcontent.length;
    	//Write webcontent into file
    	for(int i = 0; i < len; i++){
    	    String hex = Integer.toHexString((int)webcontent[i]);
